...
def transact():
    ...
    try:
        ...
        if (
            ...
            or "PhoneNumber" not in data
            or "AccountReference" not in data
        ):
            return (
                jsonify(
                    {"status": "danger", "message": "Invalid input data", "data": {}}
                ),
                400,
            )
...
def query():
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "info", "message": "Query processing", "data": response}
            ),
            200,
        )
    except Exception as e:
...
def callback():
    ...
    try:
        data = request.json
        print("Callback data:", data)  # Debugging line to check the callback data
        if not data:
            return (
                jsonify(
                    {"status": "danger", "message": "Invalid callback data", "data": {}}
                ),
                400,
            )
        ...
        return (
            jsonify(
                {"status": "info", "message": "Callback processing", "data": response}
            ),
            200,
        )
    except Exception as e:
